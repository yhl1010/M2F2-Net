...
from models.decode_heads.head import Head
...
class MCFModule(nn.Module):
    """Multi-modal Cross Fusion module"""

    def __init__(self, in_channel):
        super(MCFModule, self).__init__()
        self.gce_1 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channel, in_channel, 1, 1, 0),
            nn.Sigmoid()
        )

        self.gce_2 = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channel, in_channel, 1, 1, 0),
            nn.Sigmoid()
        )
        ...
        self.conv_feat = nn.Sequential(
            nn.Conv2d(in_channel * 2, in_channel, 1, 1, 0),
            nn.BatchNorm2d(in_channel),
            nn.LeakyReLU(0.2),
        )
    ...
    def forward(self, x, y):
        ...
        feat_x = self.conv_1(feat_x)

        feat_y = self.conv_2(feat_y)

        feat = torch.cat([feat_x, feat_y], dim=1)
        feat = self.conv_feat(feat)

        return feat_x, feat_y, feat
...
class MixVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
        ...
        self.cam = MCFModule(embed_dims[0])
    ...
    def forward_features(self, x, y):
        ...
        feat_x, feat_y, feat = self.cam(x, y)
        outs.append(feat)

        # stage 2
        x, H, W = self.patch_embed2(feat)
        ...
        return feat_x, feat_y, outs

    def forward(self, x, y):
        feat_x, feat_y, x = self.forward_features(x, y)
        seg_final, seg_img_coarse, seg_sn_coarse, edge = self.decode_head(x, feat_x, feat_y)
...
if __name__ == '__main__':
    ...
    x = torch.randn(2, 3, 704, 1280)
    y = torch.randn(2, 3, 704, 1280)
    out = net(x, y)
    pass